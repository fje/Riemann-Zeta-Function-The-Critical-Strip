...
\end{lemma}
\begin{proof}
	The change of variables $y \to \sqrt{t} y$ shows that the Fourier transform of the function $f(y) = e^{-\pi x y^2}$ is $\hat{f}(\xi) = \frac{1}{\sqrt{\xi}} e^{-\frac{\pi \xi^2}{x}}$. Apparently $f(y)$ belongs to $\mathcal{F}_a$. Thus we may apply the Poisson summation formula to the pair $\cbr{f, \hat{f}}$ and we get
\begin{equation*}
...
\begin{aligned}	
	\xi(s) 
		&= \frac{1}{2}s(1 - s) \pi^{-\frac{1}{2}s}\Gamma(\frac{1}{2}s)\zeta(s) \\
		&= \frac{1}{2}(1 - s)s \pi^{-\frac{1}{2} + \frac{1}{2}s} \Gamma(\frac{1}{2} - \frac{1}{2}s)\zeta(1 - s) = \xi(1 - s).
\end{aligned}
\end{equation*}
\end{proof}
...
\end{lemma}
\begin{proof}
	We first recall Proposition~\ref{pro:XiIdentity} which immediately yields the invariant form
\begin{equation*}
	x^{-\frac{1}{4}} \vartheta(\frac{1}{x}) = x^{\frac{1}{4}} \vartheta(x).
\end{equation*}
	Plugging this into the final integral form of the functional equation of $\xi^*$ from Proposition~\ref{pro:XiIdentity} gives
\begin{equation*}
...
\begin{equation*}
	\Im(\Xi(t)) = \lim _{\sigma \to \frac{1}{2}} \Im(\xi(\sigma + it)) = \lim _{\sigma \to \frac{1}{2}} \Im\cbr{\frac{s(s - 1)}{2}\xi^*(\sigma + it)} = 0,
\end{equation*}
...
\begin{aligned}
	\Phi(x) 
		&= -\frac{\pi \sqrt{z}}{2 i \pi} \int _{\frac{1}{4} -\infty} ^{\frac{1}{4} + \infty} \Gamma(w) \pi ^{-w} \zeta(2w) \cbr{\frac{1}{z^2}}^{-w} dw \\
		&= -\frac{\pi}{\sqrt{z}} \omega\cbr{\frac{1}{z^2}} + \frac{1}{2} \pi z.
\end{aligned}
\end{equation*}
