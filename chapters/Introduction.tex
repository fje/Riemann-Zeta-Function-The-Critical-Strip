...
\end{equation*}
	Hence we get
\begin{equation*}
	 \lim\limits_{\sigma \to 1} \abs{\zeta^3(\sigma) \zeta^4(\sigma + it_0) \zeta(\sigma + 2it_0)} = 0.
\end{equation*}
...
\end{corollary}
\begin{proof}
	This proof relies on the fact that the $\xi$ function (see Definition~\ref{def:XiDefinition}) satisfies the symmetry $\xi(s) = \xi(1 - s)$ for all $s \in \field{C}$ (see Proposition~\ref{pro:XiIdentity}). This, and the fact that only $\zeta$ can produce zeros in $\xi$, immediately yields
\begin{equation*}
